...
class Pembelian extends CI_Controller {
     ...
     public function __construct(){
			...
			if($this->session->userdata("soal") != "soal2") redirect(base_url("soal"));
	...
	public function pesanan(){
		...
		$data['nav']=$this->m_Navigasi->case2();
		$data['id'] = $this->m_Pesanbeli->get_id();
		...
		if($this->input->get("tipe")){
			...
			switch($tipe){
				...
				break;				
				case "update":
				$hasil=$this->m_Pesanbeli->update_pesanan($this->input->get("id"));
				break;
				case "delete":
	...
	public function penerimaan(){
		...
		if($this->input->get("tipe")){
			...
			switch($tipe){
				...
				break;
				case "update":
				$hasil=$this->m_Terimabarang->update_penerimaan($this->input->get("id"));
				break;
			}
	...
	public function penerimaan_ajax_harga($kode=0){
		...
		$data['nav']=$this->m_Navigasi->case2();
		$data['tid'] = $this->m_Tagihan->get_id();
		...
		if($this->input->get("tipe")){
			...
			switch($tipe){
				...
				break;				
				case "update":
				$hasil=$this->m_Tagihan->update_tagihan($this->input->get("id"));
				break;
			}
	...
	public function pembayaran(){
		...
		$data['nav']=$this->m_Navigasi->case2();
		$data['tid'] = $this->m_PembayaranB->get_id();
		...
		if($this->input->get("tipe")){
			...
			switch($tipe){
				...
				break;				
				case "update":
				$hasil=$this->m_PembayaranB->update_pembayaran($this->input->get("id"));
				break;
			}
	...
	public function laporan(){
		//load model
		$this->load->model('m_Navigasi');
		$this->load->model('m_OutputPembelian');
		
		//code
		
		//muatan data
		//muatan data
		$data["jumlah"]=($this->input->get("tipe")=="laporan") ? $this->m_OutputPembelian->jumlah() : "";
