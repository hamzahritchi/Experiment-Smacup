...
Pengiriman untuk pesanan Tuan Andy dengan ID Pesanan S003 seluruhnya dilakukan tanggal 8 September 2018 menggunakan jasa pengiriman JNE dengan nomor resi BDO001, biaya pengiriman sebesar Rp 250.000 ditanggung oleh pembeli.</p><hr>
	...
	<table class="form"">		
		...
		<tr>
			...
			<td><select  readonly name="pemesan" required><option value="" disabled selected>Silahkan Pilih</option><?php foreach($pembeli as $p){echo "<option value=\"$p->kontak_id\">$p->kontak_nama</option>";} ?></select></td>
		...
		<tr>
			...
			<td><input class="tgl" type="text" name="tanggal" autocomplete="off" required></td>
		...
		<tr>
			...
			<td><select name="term"><option value='fob_shipping_point'>FOB Ditanggung Pembeli</option><option value='fob_destination_point'>FOB Ditanggung Penjual</option></select></td>
...
Pengiriman untuk pesanan Tuan Andy dengan ID Pesanan S003 seluruhnya dilakukan tanggal 8 September 2018 menggunakan jasa pengiriman JNE dengan nomor resi BDO001, biaya pengiriman sebesar Rp 250.000 ditanggung oleh pembeli.</p><hr>
	...
	<tr>
			...
			<td><input type="text" readonly name="id" required pattern="[a-zA-Z0-9]+"></td>
		...
		<tr>
			...
			<td><select class="pesanan" class="pemesan" name="pesanan" required><option selected disabled>Silahkan pilih</option>
		...
		<tr>
			...
			<td><select name="pemesan" readonly required><option value="" diabled>Silahkan Pilih</option><?php foreach($pembeli as $p){echo "<option value=\"$p->kontak_id\">$p->kontak_nama</option>";} ?></select></td>
		...
		<tr>
			...
			<td><input class="tgl" type="text" name="tanggal" required autocomplete="false"></td>
	 ...
	 <div class="table-responsive">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary">Ubah</button> <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
...
<table class='table' id="ajaxtable">
	...
	<tobdy>
		...
		<?php foreach($pengiriman as $p){
			...
			echo $p->pengiriman_term == "fob_shipping_point" ? "<td>FOB Ditanggung Pembeli</td>" : "<td>FOB Ditanggung Penjual</td>";
			...
			if($p->pengiriman_status == 0 )

			echo "<a href='#' data-id='$p->pengiriman_id' data-toggle='modal' data-target='#editPengiriman' class='editButton btn btn-default glyphicon glyphicon-eye-open'></a></td>";
...
$(document).ready(function() {
    ...
    $('.editButton').on('click', function() {
        ...
        $.ajax({
            ...
            $('#editPengiriman')
                ...
                .find('form').attr("action","?tipe=update&id="+response.pengiriman_id).end();
    ...
    $('#createPengiriman .pesanan').on('change', function() {

        // tarik record

        var id = $("#createPengiriman .pesanan").val();
    ...
    $('#editPengiriman .pesanan').on('change', function() {

        // tarik record

        var id = $("#editPengiriman .pesanan").val();
