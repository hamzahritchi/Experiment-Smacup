...
<div id="createPenerimaan" class="modal fade" role="dialog">
...
PT. Kain katun memberikan resi pengiriman atas pesanan P005 tanggal 12 Oktober 2018. Nomor resi atas penggiriman tersebut adalah JKT001 dengan biaya pengiriman sebesar Rp 100.000. Pengiriman dilakukan untuk 500 meter kain yang dipesan oleh Konveksi Baju Baru.  </p><hr>
...
PT. Kain katun memberikan resi pengiriman atas pesanan P005 tanggal 12 Oktober 2018. Nomor resi atas penggiriman tersebut adalah JKT001 dengan biaya pengiriman sebesar Rp 100.000. Pengiriman dilakukan untuk 500 meter kain yang dipesan oleh Konveksi Baju Baru. </p><hr>
        <h4 class="modal-title">Edit Penerimaan Barang</h4>
      </div>
	  <form method="post" action="?tipe=tambah">
		<div class="modal-body">
	<table class="form"">		
...
?>
	...
	<div class="row">
		<div class="col-sm-3 col-xs-3">
			<button style="font-size:14px;padding:5px 5px;" type="button" class="btn btn-primary" data-toggle="modal" data-target="#createPenerimaan">+ Add Penerimaan</button>
		</div>
	</div>
...
<table class='table' id="ajaxtable">
	...
	<tobdy>
		<?php foreach($terima as $p){
			...
			echo $p->terimabarang_term=="fob_shipping_point" ? "<td>FOB Ditanggung Pembeli</td>" : "<td>FOB Ditanggung Penjual</td>";
			echo $p->terimabarang_status=="1" ? "<td>Sudah Diproses</td>" : "<td>Belum Diproses</td><td>";
			if($p->terimabarang_status == 0)
			echo "
...
$(document).ready(function() {
    ...
    $('#createPenerimaan .pesanan').on('change', function() {
        // tarik record
        var id = $("#createPenerimaan .pesanan").val();
    ...
    $('#editPenerimaan .pesanan').on('change', function() {
        // tarik record
        var id = $("#editPenerimaan .pesanan").val();
    ...
    $('.editButton').on('click', function() {
        ...
        $.ajax({
            ...
            $('#editPenerimaan')
				...
				.find('[name="biaya"]').val(response.terimabarang_beban).end()
                .find('form').attr("action","?tipe=update&id="+response.terimabarang_id).end()
			}
		});//end ajax header
