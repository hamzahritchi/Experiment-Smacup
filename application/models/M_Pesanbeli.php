...
class m_Pesanbeli extends CI_Model {
	...
	public function update_pesanan($kode = "A123"){
        ...
        $this->form_validation->set_rules('term', 'id', 'trim|required|max_length[30]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			if($case2_a2 == 1) return ["gagal","Anda sudah mengerjakan soal selanjutnya"];
			...
			if($this->ms->cek_ada("eksperimen_pesanbeli",["pesanbeli_id"=>$id,"peserta_id"=>$peserta])==TRUE && $kode != $id) return ["gagal","ID Sudah dipakai"];





			if($this->ms->cek_ada("eksperimen_pesanbeli",["pesanbeli_id"=>$kode,"peserta_id"=>$peserta])==FALSE) return ["gagal","ID Tidak Ada"];
			...
			$this->db->where(["pesanbeli_id"=>$kode,"peserta_id"=>$peserta]);

			$this->db->update("eksperimen_pesanbeli",$data);



			$this->db->where(["pesanbeli_id"=>$kode,"peserta_id"=>$peserta])->delete("eksperimen_barispesanbeli");
	...
	public function delete_pesanan($kode){
		...
		if($case2_a1 == 1) return ["gagal","Anda sudah mengerjakan soal ini, silahkan gunakan P005 untuk soal penerimaan barang"];
