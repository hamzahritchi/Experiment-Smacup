...
class M_Partisipan extends CI_Model {	
	...
	{
		//Menarik library dan fungsi pembantu
		$this->load->database();
		$this->load->library('user_agent');
		$this->load->helper('cookie');

		//data peserta - set variable
		...
		//cek ip yang masuk benar atau salah
		if(!filter_var($ip, FILTER_VALIDATE_IP)) 
			$cekIP = "";
		else
			$cekIP = $ip."/";

		//tarik data ip dari jasa pelacak IP, kode bisa berubah sesuai API jasa tersebut
		//Apabila berhasil ditarik, beberapa variabel diisi dari data jasa tersebut
		...
		$maxDev = 1; //maksimal standar deviasi
		...
		// Akan terus diacak hingga mendapat nilai yang harus diisi
		if($deviasi > $maxDev){
			while(1){
				if($random <= $cx1){
					if($px1 == $minpx){
