...
class m_Pesanan extends CI_Model {
...

    {
        ...
        $this->form_validation->set_rules('term', 'id', 'trim|required|max_length[40]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			if($case1_a1 == 1) return ["gagal","Anda sudah mengerjakan soal ini, silahkan edit data untuk mengkoreksi jawaban"];
			...
			if($tanggal == "2018-09-05") $n3 = 1; //tanggal
			...
			for($i=0;$i<count($barang);$i++){
				...
				if($this->ms->cek_ada("eksperimen_barispesanan",array("peserta_id"=>$peserta,"barang_id"=>$barang[$i],"pesanan_id"=>$id))){
	...
	public function update_pesanan($kode = "A123")

    {
        ...
        $this->form_validation->set_rules('term', 'id', 'trim|required|max_length[40]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			if($case1_a2 == 1) return ["gagal","Anda sudah mengerjakan soal selanjutnya"];
			...
			if($this->ms->cek_ada("eksperimen_pesanan",["pesanan_id"=>$id,"peserta_id"=>$peserta])==TRUE && $kode != $id) return ["gagal","ID Pesanan telah dipakai $id"];



			if($this->ms->cek_ada("eksperimen_pesanan",["pesanan_id"=>$kode,"peserta_id"=>$peserta])==FALSE) return ["gagal","ID Pesanan tidak ada"];
			...
			$this->db->where(["peserta_id"=>$peserta,"pesanan_id"=>$kode]);

			$this->db->update("eksperimen_pesanan",$data);



			$this->db->where(["pesanan_id"=>$kode,"peserta_id"=>$peserta])->delete("eksperimen_barispesanan");
			...
			return array("berhasil","Pesanan berhasil diupdate");
	...
	public function delete_pesanan($kode){
		...
		if($case1_a1 == 1) return ["gagal","Anda sudah mengerjakan soal ini, silahkan gunakan S003 pada soal pengiriman barang"];
