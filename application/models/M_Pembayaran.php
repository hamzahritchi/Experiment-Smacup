...
class m_pembayaran extends CI_Model {	
	...
	public function update_pembayaran($kode)
    {
		$this->load->database();
		$this->load->model('m_Serbaguna','ms');
        $this->load->library('form_validation');
        ...
        $this->form_validation->set_rules('id', 'ID', 'trim|required|max_length[20]');
		 ...
		 if($this->form_validation->run() == FALSE) return "gagal";
			...
			if($this->ms->cek_ada("eksperimen_pembayaran",["pembayaran_id"=>$kode,"peserta_id"=>$peserta])==FALSE) return ["gagal","ID Pembayaran tidak ada"];


			if($this->ms->cek_ada("eksperimen_pembayaran",["pembayaran_id"=>$this->input->post("id",true),"peserta_id"=>$peserta])==TRUE && $this->input->post("id",true) != $kode) return ["gagal","ID Pembayaran telah dipakai"];
			...
			if($this->input->post("id",true) == "CR002"){
				if($this->input->post("tanggal") == "2018-09-15") $n15 = 1;
				if($this->input->post("kwitansi") == "SI003") $n16 = 1;
				if($this->input->post("bayar") == "25250000") $n17 = 1;
				if($this->input->post("via") == "bank") $n18 = 1;
			...
			$this->db->where(["pembayaran_id"=>$kode,"peserta_id"=>$peserta]);
			$this->db->update("eksperimen_pembayaran",$data);
