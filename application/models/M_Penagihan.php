...
class m_Penagihan extends CI_Model {
        ...
        $this->form_validation->set_rules('id', 'id', 'trim|required|max_length[20]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			for($i=0;$i<count($barang);$i++){
				...
				if($this->ms->cek_ada("eksperimen_bariskwitansi",array("barang_id"=>$barang[$i],"kwitansi_id"=>$id,"peserta_id"=>$peserta))){

					$this->db->where("barang_id",$barang[$i])->where(["kwitansi_id"=>$id,"peserta_id"=>$peserta])->set("bariskwitansi_jumlah","bariskwitansi_jumlah+".$jumlah[$i],false)->set("bariskwitansi_subtotal","bariskwitansi_subtotal +".($jumlah[$i]*$dbarang->barang_hargajual),false)->update("eksperimen_bariskwitansi");
	...
	public function update_penagihan($kode = "A123")
        ...
        $this->form_validation->set_rules('id', 'id', 'trim|required|max_length[20]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			$case1_a4 = $this->db->where("peserta_id",$peserta)->get("eksperimen_case1")->row()->case1_a4;
				...
				if($case1_a4 == 1) return ["gagal","Soal selanjutnya telah dikerjakan"];
			...
			if($this->ms->cek_ada("eksperimen_kwitansi",["kwitansi_id"=>$id,"peserta_id"=>$peserta])==TRUE && $kode != $id) return ["gagal","No Kwitansi telah dipakai"];



			if($this->ms->cek_ada("eksperimen_kwitansi",["kwitansi_id"=>$kode,"peserta_id"=>$peserta])==FALSE) return ["gagal","No Kwitansi tidak ada"];
			...
			$this->db->where(["kwitansi_id"=>$kode,"peserta_id"=>$peserta]);

			$this->db->update("eksperimen_kwitansi",$data);



			$this->db->where(["kwitansi_id"=>$kode,"peserta_id"=>$peserta])->delete("eksperimen_bariskwitansi");
