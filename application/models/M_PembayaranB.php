...
class m_pembayaranb extends CI_Model {	
        ...
        $this->form_validation->set_rules('id', 'ID', 'trim|required|max_length[20]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			if(empty($tagihan)){
				...
				if($this->ms->cek_ada("eksperimen_tagihan",["tagihan_id"=>$tagihan,"peserta_id"=>$peserta])==FALSE) return ["gagal","Tagihan tidak ada"];
			}
	...
	public function update_pembayaran($kode="A123")
    {
		//validasi
        $this->load->library('form_validation');
		$this->load->model('m_Serbaguna','ms');
        ...
        $this->form_validation->set_rules('id', 'ID', 'trim|required|max_length[20]');
		 ...
		 if($this->form_validation->run() == FALSE){
			...
			if($this->ms->cek_ada("eksperimen_pembtagihan",["pembtagihan_id"=>$kode,"peserta_id"=>$peserta])==FALSE) return ["gagal","ID Tidak ada"];
			...
			if($this->input->post("id",true) == "CD003"){
				if($this->input->post("tanggal") == "2018-10-22") $n15 = 1;
				if($this->input->post("tagihan") == "PI004") $n16 = 1;
				if($this->input->post("bayar") == "37500000") $n17 = 1;
				if($this->input->post("via") == "tunai") $n18 = 1;
			...
			$this->db->where(["pembtagihan_id"=>$kode,"peserta_id"=>$peserta]);
			$this->db->update("eksperimen_pembtagihan",$data);
