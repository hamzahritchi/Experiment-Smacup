...
<div id="createPengiriman" class="modal fade" role="dialog">
	...
	<tr>
		...
		<tr>
			<td>ID Pesanan</td>
			<td><input type="text" class="pesanan" name="pesanan" required pattern="[a-zA-Z0-9]+"><a class="pilih btn btn-default" href='#'>Use</a></td>
		</tr>
		<tr>
			<td>Tanggal</td>
...
<div id="editPenerimaan" class="modal fade areaprint" role="dialog">
	...
	<tr>
		...
		<tr>
			<td>Tanggal</td>
			<td><input type="date" disabled name="tanggal" required max="<?php echo $this->session->userdata("periode_sampai") ?>" min="<?php echo $this->session->userdata("periode_dari") ?>" value="<?php echo date('Y-m-d'); ?>"></td>
		</tr>	
	 </table>
	 <br><br>
        ...
        $.ajax({
            ...
            // Populate the form fields with the data returned from server
		      ...
		      //alert(value);
				  ...
				  akun.append($("<td><input disabled type='text' value='"+produk+"' list='barang' class='long changeble' autocomplete='off' name='namabarang[]' placeholder='Nama Produk'></td>"))
