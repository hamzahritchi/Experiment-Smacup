...
<div id="createPesanan" class="modal fade" role="dialog">
	...
	<tr>
		...
		<tr>
			<td>Term</td>
			<td><select required name="term"><option value='fob_shipping_point'>FOB Shipping Point</option><option value='fob_destination_point'>FOB Destination Point</option></select></td>
		</tr>		
	 </table>
	 <br><br>
...
<div id="editPesanan" class="modal fade areaprint" role="dialog">
	...
	<tr>
			<td>ID</td>
			<td><input type="text" name="id" required pattern="[a-zA-Z0-9]+" disabled ></td>
		</tr>		
		<tr>
			<td>Vendor</td>
			<td><select name="vendor" readonly disabled><option value="" diabled>Silahkan Pilih</option><?php foreach($pembeli as $p){echo "<option value=\"$p->kontak_id\">$p->kontak_nama</option>";} ?></select></td>
		</tr>
		<tr>
			<td>Tanggal</td>
			<td><input type="date" name="tanggal" disabled  required max="<?php echo $this->session->userdata("periode_sampai") ?>" min="<?php echo $this->session->userdata("periode_dari") ?>" value="<?php echo date('Y-m-d'); ?>"></td>
		</tr>	
		<tr>
			<td>Term</td>
...
<table class='table' id="ajaxtable">
	...
	<tbody>
		<?php foreach($pesanan as $p){
			...
			echo $p->pesanbeli_term=="fob_shipping_point" ? "<td>FOB Shipping Point</td>" : "<td>FOB Destination Point</td>";
			...
			echo "<td>
							<a href='#' data-id='$p->pesanbeli_id' data-toggle='modal' data-target='#editPesanan' class='editButton btn btn-default glyphicon glyphicon-eye-open'>
							</a>
							<a href='".base_url()."pembelian/pesanan/?tipe=delete&id=$p->pesanbeli_id' onclick=\"return confirm('Anda yakin?')\" class='btn btn-default glyphicon glyphicon-trash'>
							</a>
				</td>";
			echo "</tr>";
		}
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            // Populate the form fields with the data returned from server
		      ...
		      //alert(value);
				  ...
				  akun.append($("<td><input type='text' disabled value='"+produk+"' list='barang' class='long changeble' autocomplete='off' name='namabarang[]' placeholder='Nama Produk'></td>"))
    ...
    $(document).on('input',".changeble",function() {
